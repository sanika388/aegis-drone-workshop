...
export default function HomePage() {
  ...
  const notice = workshop?.notice || '';
  const posterUrl = workshop?.homepage_poster_url || '/poster.png';
  ...
  return (
    <div className="min-h-screen bg-[#050505] text-white flex flex-col justify-between">
      {/* Top Notice Dispatch Banner */}
      {notice && (
        <div className="bg-[#0e1610] border-b border-neon/30 px-4 py-2.5 flex items-center justify-center gap-2 text-xs font-mono text-neon">
          <span className="inline-block w-2 h-2 rounded-full bg-neon animate-ping shrink-0" />
          <span className="font-bold tracking-wider uppercase">[LAB DISPATCH]</span>
          <span className="text-gray-300 font-sans">{notice}</span>
        </div>
      )}

      <div className="max-w-7xl mx-auto px-6 py-16 grid grid-cols-1 lg:grid-cols-12 gap-12 items-center flex-1">
        ...
        {/* Dynamic Workshop Banner Container */}
