...
export default function AttendeeRegistryTab({
  ...
  const permanentPurgeAttendee = async (attendee: Attendee) => {
    ...
    const confirmPurge = window.confirm(
      `⚠️ PERMANENT PURGE: Delete ${attendee.name} (${displayId}) permanently? This cannot be undone.`
    );
  ...
  const handleManualSpotIntake = async (e: React.FormEvent) => {
    ...
    try {
      // Direct insert (the trigger will auto-assign sequential clearance_id & batch)
      ...
      const clearanceId = newEntry.clearance_id || newEntry.id;
